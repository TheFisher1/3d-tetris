...
use crate::game::game_elements::EMPTY_CELL;
use bevy::prelude::*;
...
impl GameGrid {
    ...
    pub fn is_row_occupied(&self, y: i32) -> bool {
        if y == 0 {
            return true;
        }
    ...
    pub fn clear_row(&mut self, y: i32) {
        ...
        (1..GRID_DEPTH).for_each(|x| {
            (1..GRID_WIDTH).for_each(|z| {
                self.set_cell(x, y, z, EMPTY_CELL);
            })
        })
    }
